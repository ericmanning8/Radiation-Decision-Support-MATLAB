...
function emdValue = emdCalculator(patient1, patient2, roiName)

[queryDistance1 queryVolume1] = mysql(horzcat('SELECT ovhDistance_ptv1, ovhVolume_ptv1 FROM structure_set_roi_sequence_copy WHERE fk_patient_id=',num2str(patient1),' AND stdROIName="',roiName,'"'));
[queryDistance2 queryVolume2] = mysql(horzcat('SELECT ovhDistance_ptv1, ovhVolume_ptv1 FROM structure_set_roi_sequence_copy WHERE fk_patient_id=',num2str(patient2),' AND stdROIName="',roiName,'"'));
