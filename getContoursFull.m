% This function returns a 3D contour block and solid ROI mask. It assumes
% that the structure file and related CT images are in the same sub-folder.
...
containing_folder = strcat('UCLA_PR_',num2str(patient_number));%folder containing DCM files 
imageList = dir(fullfile(containing_folder, 'CT.*.dcm'));%List all the CT images in the sub-folder
...
for planeIndex = 1:roiNumPlanes % for each contour plane of the selected ROI
    ...
    imagei = dicominfo(strcat(containing_folder,'/CT.',ImageSOP,'.dcm'));% get dicom metadata of associated image
    ...
    temp=length(planeContourData)/3;%number of x points, and therefore, also the number of y points and number of z points
    ...
    for i=1:3:x 
        ...
        b=[px-sx; py-sy];
        v=A\b; %(note: backward slash) Solve the linear equation
