...
%Get the PTV Block separately and create a mask for surface outlines only
ptvBlock=HN_getContoursFull(patient_id, 'ptvAll');
...
for i=1:length(stdROINames) %For each ROI in the patient's ROI Sequence
    %if (~strcmp(stdROINames{i},'ptvAll'))%execute only if the ROI is not a PTV
     if (strcmp(stdROINames{i},'eyeRt'))
        ...
        if (numIntersectingRoiVoxels~=0) 
            for oarVoxel = 1:numIntersectingRoiVoxels
                ...
                for ptvVoxel = 1:numPtvOutlineVoxels
                    distance = sqrt((ptvOutlineSub(ptvVoxel,1)-roiIntersectingSub(oarVoxel,1))^2+(alpha*(ptvOutlineSub(ptvVoxel,2)-roiIntersectingSub(oarVoxel,2)))^2+(beta*(ptvOutlineSub(ptvVoxel,3)-roiIntersectingSub(oarVoxel,3)))^2);
        ...
        if (numNonIntersectingRoiVoxels~=0)
            for oarVoxel = 1:numNonIntersectingRoiVoxels
                minimumDistance=1000000;
                for ptvVoxel = 1:numPtvOutlineVoxels
                    distance = sqrt((ptvOutlineSub(ptvVoxel,1)-roiNonIntersectingSub(oarVoxel,1))^2+(alpha*(ptvOutlineSub(ptvVoxel,2)-roiNonIntersectingSub(oarVoxel,2)))^2+(beta*(ptvOutlineSub(ptvVoxel,3)-roiNonIntersectingSub(oarVoxel,3)))^2);
                    minimumDistance = min(minimumDistance, distance);
                end
    ...
    % %% CALCULATE OVH PARAMETERS AND PLOT OVH
        ...
        %mysql(horzcat('UPDATE structure_set_roi_sequence SET Volume="',num2str(roiVolume),'", OverlapVolume="',num2str(roiIntersectingVolume),'", PercentageOverlap_ROI_Fraction="',num2str(roiPercentageOverlap_OAR_Fraction),'", PercentageOverlap_Total_Fraction="',num2str(roiPercentageOverlap_Total_Fraction),'" WHERE id="',num2str(structure_set_roi_sequence_ids(i)),'"'));
        ...
        %mysql(horzcat('UPDATE structure_set_roi_sequence SET ovhComplete = "',ovhComplete,'" WHERE id="',num2str(structure_set_roi_sequence_ids(i)),'"'));
...
% legend(stdROINames,'Location','BestOutside');
